...
├──────────────────────────────────┤
│ 💬Chat │📝Diff │📋Tasks│📁Repos│⚙│  ← 底部導航（5 tabs）
└──────────────────────────────────┘

底部導航列（固定，5 tabs）:

┌─────────┬─────────┬─────────┬─────────┬─────────┐
│ 💬 Chat │ 📝 Diff │ 📋 Tasks│ 📁 Repos│ ⚙ Set.  │
│         │  (2)    │  (1)    │         │         │
└─────────┴─────────┴─────────┴─────────┴─────────┘
...
├──────────────────────────────┤
│  ┌──────────────────────┐ ⚡ │  ← 輸入區域
│  │ Type a message...     │   │     ⚡ = Quick Actions（git 操作面板）
│  └──────────────────────┘ ▶  │     ▶ = 送出
│                              │
├──────────────────────────────┤
│ 💬Chat │📝Diff │📋Tasks│📁│⚙│  ← 底部導航（5 tabs）
└──────────────────────────────┘
```
...
### 3. Tasks Page (Phase 2 — 骨架已建，功能未實作)
...
**目前狀態**：頁面僅顯示靜態空狀態文字（"No queued/running/completed tasks"）。
Kanban UI、Task 建立表單、Task runner 均為 Phase 2 內容，尚未實作。
DB table `tasks` 已建立，待 Phase 2 接上。
...
### 5. Settings Page（從 BottomNav ⚙ 進入）
...
已實作的元件標記 ✅，未實作標記 ❌（Phase 2+）。

```
App ✅
├── ToastContainer ✅                   # 全域 Toast 通知
└── Routes
    ├── AppLayout ✅                    # 統一 wrapper（WorkspaceTabs + BottomNav）
    │   ├── WorkspaceTabs ✅            # 橫向滾動 tab bar（>1 workspace 時顯示）
    │   ├── {children}                  # 頁面內容
    │   └── BottomNav ✅                # 底部 5-tab 導覽（Chat/Diff/Tasks/Repos/Settings）
    │
    ├── ChatPage ✅
    │   ├── Header ✅                   # workspace 名 + conversation title + [+] 新對話
    │   ├── ConversationSelector ✅     # BottomSheet 切換/刪除對話（兩步驟刪除確認）
    │   ├── MessageList ✅              # 可捲動的訊息列表
    │   │   └── MessageBubble ✅        # 單一訊息（含 markdown + code block）
    │   ├── ToolApprovalCard ✅         # AI tool use 審批
    │   ├── TokenUsageCard ✅           # Token 使用量顯示
    │   ├── ChatInput ✅                # 文字輸入 + 送出
    │   └── QuickActions ✅             # Git 操作 BottomSheet（Stage/Commit/Push/Pull）
    │
    ├── DiffPage ✅
    │   ├── Header ✅                   # 統計摘要（點擊開啟檔案列表 BottomSheet）
    │   ├── FileList BottomSheet ✅     # 檔案選擇
    │   ├── DiffViewer ✅               # Unified diff + Prism 語法高亮
    │   └── ReviewActions ✅            # Approve / Reject / Approve All
    │
    ├── TasksPage ✅ (骨架)
    │   └── 靜態空狀態文字              # Phase 2: KanbanBoard、TaskCard、NewTaskSheet
    │
    ├── ReposPage ✅
    │   ├── Workspace 列表 ✅           # 卡片式 workspace 列表
    │   ├── 註冊 Modal ✅               # 路徑掃描 + 手動輸入
    │   └── QuickActions ✅             # Git 操作面板
    │
    └── SettingsPage ✅
        ├── AI Model 選擇 ✅            # Sonnet/Opus（localStorage）
        └── Push Notification toggle ✅ # Web Push 訂閱/取消

未實作的通用元件（Phase 2+）:
    ❌ StatusBar                        # 斷線狀態指示條
    ❌ ConfirmDialog                    # 通用確認對話框
    ❌ PullToRefresh                    # 下拉刷新手勢
    ❌ EmptyState                       # 通用空狀態元件
    ❌ VoiceButton                      # 語音輸入按鈕（hook 已建，UI 未完成）
    ❌ AttachButton                     # 附加檔案按鈕
    ❌ TemplateBar                      # Prompt template 選擇列（DB table 已建）
    ❌ ContextFileSheet + FileTree      # 檔案選擇 BottomSheet
```
