...
| 指標 | 目標值 | 驗證方式 | 檢查時間 |
|------|--------|----------|----------|
| Workspace 切換無資料遺失 | 返回原 workspace 後 chat history 完整 | 手動測試 | 已驗證 |
| 7 個 store 均使用 per-workspace pattern | 含 `ByWorkspace: Record<string, ...>` | `grep "ByWorkspace" client/src/stores/*.ts` | 已驗證 |
| 背景 workspace 持續接收 WS 事件 | Unread badge 正確顯示 | 手動測試 | 已驗證 |
