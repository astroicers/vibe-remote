...
| 指標 | 目標值 | 驗證方式 | 檢查時間 |
|------|--------|----------|----------|
| Task CRUD API 完整 | POST/GET/PATCH/DELETE 可用 | `tasks.test.ts` | 已驗證 |
| Priority ordering 正確 | urgent > high > normal > low | 單元測試 | 已驗證 |
| 同時只執行一個 task | `isProcessing` flag 阻擋並行 | Code review | 已驗證 |
