...
| 指標 | 目標值 | 驗證方式 | 檢查時間 |
|------|--------|----------|----------|
| Pairing session 5 分鐘過期 | 過期自動清理 | 單元測試 | 已驗證 |
| Dev quick-pair 僅 dev 可用 | production 時 endpoint 不存在 | 測試 | 已驗證 |
| 每次請求更新 `last_seen_at` | authMiddleware 含 UPDATE | Code review | 已驗證 |
